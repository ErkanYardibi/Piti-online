    ...
    if (url.pathname === '/api/demo-admin') {
      ...
      try {
        ...
        const upstream = await fetch('https://ldufxzwgwbaogpmwqhlw.supabase.co/functions/v1/demo-admin', {
          method:'POST', headers:{'apikey':'sb_publishable_zmPhdwrGsRLY1B0XYb2ssg_8_qE67D7','Authorization':authorization,'Content-Type':'application/json','Origin':url.origin}, body, signal:AbortSignal.timeout(35000), redirect:'manual'
        });
        if (upstream.status >= 300 && upstream.status < 400) return json(502, 'DEMO servisi beklenmeyen bir yönlendirme döndürdü.');
