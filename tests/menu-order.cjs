...
d.querySelector('[data-menu-move="messages"][data-direction="-1"]').click();assert.equal(d.querySelector('#saveMenuOrder'),null);await Promise.resolve();assert.equal(nav()[0],'messages');
...
d.querySelector('[data-menu-move="messages"][data-direction="-1"]').click();assert.equal(nav()[0],'messages','member can reorder too');
w.testRun("state.role='pt';render()");await d.querySelector('#resetMenuOrder').onclick();assert.equal(nav()[0],'dashboard');
...
w.testRun("window.release=[];syncCloudDataNow=()=>new Promise(resolve=>window.release.push(resolve));state.page='appearance';render()");
const theme=d.querySelector('#appearanceTheme'),background=d.querySelector('#appearanceBackground');assert.equal(d.querySelector('#saveAppearance'),null);
theme.value='dark';const first=theme.onchange();assert.equal(d.documentElement.dataset.theme,'dark','theme applies before network completes');await new Promise(r=>setTimeout(r,0));
theme.value='light';background.value='navy';const second=background.onchange();assert.equal(d.documentElement.dataset.theme,'light');assert.equal(d.documentElement.style.getPropertyValue('--bg'),'#e8eef8');
w.release[0]();await first;await new Promise(r=>setTimeout(r,0));assert.equal(d.documentElement.dataset.theme,'light','old save cannot overwrite latest preview');w.release[1]();await second;assert.equal(w.testRun('state.appearance.theme'),'light');assert.equal(w.testRun('state.appearance.background'),'navy');
w.testRun("syncCloudDataNow=async()=>{throw Error('offline test')}");theme.value='dark';await theme.onchange();assert.equal(d.documentElement.dataset.theme,'light');assert.equal(theme.value,'light');assert.match(d.querySelector('#appearanceStatus').textContent,/Kaydedilemedi/);
